...
export default function PortfolioWidget({isMyProfile, trueUser}: Props) {
    ...
    return (
        <section className="glass-effect rounded-[18px] p-6">
            ...
            <main className="flex flex-col gap-4">
                ...
                {
                    !Array.isArray(documents) && !isLoading &&
                    <p className="text-text-main font-semibold">
                        {isMyProfile ?
                            "Вы не загрузили ни одного документа :(" :
                            !Boolean(trueUser?.publication?.is_uploaded) ? "Пользователь не загрузил ни одного документа :(" : ""
                        }
                    </p>
                }
