...
export default function UpdateDocument({document_values, document_id}: Props) {
    ...
    const {
        ...
        Tags,
        tags
    } = useTags({tagsInitial: document_values?.categories || documentRecognized?.categories || null})
    ...
    async function handleSubmit(e) {
        await updateDocument({e, setErrors, formRef, tags, checked, document_id})
    }

    const handleDeleteDocument = useCallback(async () => {
        await deleteDocument({document_id: document_id})
    }, [])
    ...
    return (
        <form className="relative flex flex-col gap-4" onSubmit={handleSubmit} ref={formRef}>
            ...
            <button
                type="button"
                className="
                /* Позиционирование: всегда внизу по центру */
                ...
                /* Фон: Глубокое стекло и градиент */
                backdrop-blur-2xl
                ...
                hover:shadow-[0_20px_60px_rgba(var(--brand-rgb),0.5)]
                active:scale-[0.98]
            ...
            >
                ...
                <span
                    ...
                    Лимит: <b className="text-white font-mono">{user?.access?.recognizes_left || 0}</b>
                </span>
                </div>
