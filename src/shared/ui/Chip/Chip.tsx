...
export default function Chip({Icon, text, className, isActive=false, ...props}: Props) { // Все остальное летит в Link
    return (
        <Link
            {...props}
            className={`${isActive && "glass-effect"} w-full flex gap-3 items-center group transition-all duration-300 rounded-[20px]  cursor-pointer ${className}`}
        >
            ...
            <p className="text-text-muted font-semibold text-secondary group-hover:text-text-main">{text}</p>
        </Link>
    )
}