...
export default function City({ next }) {
    // Используем пустую строку вместо null для начального значения
    ...
    async function handleSubmit(e) {
        ...
        if (cityId) {
            result.city_id = String(cityId);
        } else {
            if (city.trim()) { // Проверяем, что город не пустой
                result.city = city;
            }
        }
    ...
    useEffect(() => {
        // Сохраняем в localStorage только если есть значение
        if (city !== null && city !== undefined) {
            localStorage.setItem("city", city);
        }

        // Проверяем длину только если city - строка
    ...
    useEffect(() => {
        ...
        // Загружаем города если есть cityId (опционально)
        // Убрал или оставь в зависимости от логики приложения
        /*
        const timeout = setTimeout(async () => {
            setLoading(true);
            const response = await $fetch(`cities?city=${city}`);
            setCities(response?.json?.cities || []);
            setLoading(false);
        }, 500);
